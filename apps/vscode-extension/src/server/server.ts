...
import { check, explain, InMemoryTupleRepository, parseTuple, type RelationTuple } from '@maroonedsoftware/permissions';
...
const validateWithSiblings = (
  schema: string,
  filename: string,
): { file: FileNode; lowered?: ReturnType<typeof validateFile>['lowered']; error?: CompileError; localNames: Set<string> } => {
...
const loadRequest = (
  ...
  repo: InMemoryTupleRepository;
  tuples: RelationTuple[];
  checkTuple: RelationTuple;
  model: NonNullable<ReturnType<typeof validateFile>['lowered']>['model'];
} => {
