...
export class PlaygroundPanel {
  ...
  private constructor(
    ...
    panel.webview.onDidReceiveMessage((msg: PlaygroundMessage) => this.handleMessage(msg), undefined, this.disposables);
    ...
    vscode.window.onDidChangeActiveTextEditor(() => void this.refreshFromActiveEditor(), undefined, this.disposables);
  }
  ...
  private renderHtml(): string {
    ...
    const scriptUri = webview.asWebviewUri(vscode.Uri.file(this.context.asAbsolutePath(path.join('dist', 'client', 'playground', 'webview.js'))));
    const styleUri = webview.asWebviewUri(vscode.Uri.file(this.context.asAbsolutePath(path.join('dist', 'client', 'playground', 'webview.css'))));
