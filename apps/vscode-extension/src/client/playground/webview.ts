...
import type { DiscoveredCheck, ExplainResponse, PlaygroundMessage, SchemaRelation, SchemaSummary } from '../../shared/playground.protocol.js';
...
const renderRelValidation = (): void => {
  ...
  const detail = failures.map(f => `  line ${f.line}: ${escape(f.message)}`).join('\n');
  ...
  switch (trace.kind) {
    case 'direct': {
      ...
      if (trace.matched) {
        ...
        const sub =
          s.kind === 'concrete' ? `${s.namespace}:${s.id}` : s.kind === 'wildcard' ? `${s.namespace}.*` : `${s.namespace}:${s.id}.${s.relation}`;
...
const pickSubject = (subjects: string[]): { ns: string; kind: 'concrete' | 'wildcard' | 'userset'; rel?: string } | undefined => {
  for (const s of subjects) {
