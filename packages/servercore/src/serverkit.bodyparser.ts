...
/**
 * What {@link ServerKitBodyParser.parse} reads from: the raw Node request, or any object that
 * carries one as `req` (a Koa context, for instance). Accepting both keeps the dispatcher
 * framework-neutral without breaking callers that hand it a context.
 */
export type ServerKitBodySource = IncomingMessage | { req: IncomingMessage };
 ...
 *
 * `type-is` returns the concrete matched type — not the pattern key — when a request
 * matches a wildcard registration, so a direct `Map.get(matched)` misses. This
...
export class ServerKitBodyParser {
  /** The registered MIME patterns, deduplicated; the same list `parse` matches against. */
   ...
   *
   * Matching uses `type-is` directly, the same library Koa's request `is()` wraps, so wildcard
   * and `+json` suffix semantics are identical whichever framework hands in the request.
   *
   * @param source - The raw request, or an object carrying it as `req` (e.g. a Koa context).
   ...
   */
  async parse(source: ServerKitBodySource): Promise<ServerKitParserResult> {
    const req = 'req' in source ? source.req : source;
    const mimeType = typeis(req, this.mimeTypes);
    if (!mimeType) {
