...
import { handleServerFeed, serverFeedFilterFromQuery, type ServerFeedContext } from '../src/serverfeed/server.feed.stream.js';
...
describe('handleServerFeed', () => {
  ...
  it('runs the hijack hook of a non-Koa context before writing', () => {
    const bus = new ServerFeed();
    const res = fakeRes();
    const order: string[] = [];
    ...
    handleServerFeed({ ...fakeCtx(res), hijack: () => order.push('hijack') }, bus, { heartbeatMs: 0 });
