...
describe('EmailFactorService', () => {
  ...
  describe('verifyEmailChallenge', () => {
    ...
    it('increments the attempt counter and persists it after a failed verification', async () => {
      ...
      await expect(service.verifyEmailChallenge('chal-id-1', 'wrong')).rejects.toMatchObject({ statusCode: 400 });

      expect(statefulCache.update).toHaveBeenCalledWith('email_factor_challenge_chal-id-1', expect.any(String));
      const stored = await statefulCache.get('email_factor_challenge_chal-id-1');
    ...
    it('locks out the challenge after the attempt budget is exhausted', async () => {
      ...
      await expect(service.verifyEmailChallenge('chal-id-1', 'wrong')).rejects.toMatchObject({ statusCode: 429 });
      expect(await statefulCache.get('email_factor_challenge_chal-id-1')).toBeNull();
