...
- ba6e34c: A consent can grant a scope of its own.
...
- 5061396: Give each email verification method its own pending-challenge slot, so an OTP and a magic link can
...
- Updated dependencies [97a75be]
  ...
  - @maroonedsoftware/encryption@0.5.13
...
- Updated dependencies [7587006]
  ...
  - @maroonedsoftware/encryption@0.5.12
...
- Updated dependencies [e2e968d]
  ...
  - @maroonedsoftware/encryption@0.5.11
...
- Updated dependencies
  ...
  - @maroonedsoftware/encryption@0.5.9
...
- Updated dependencies
  ...
  - @maroonedsoftware/encryption@0.5.8
...
- Updated dependencies
  ...
  - @maroonedsoftware/encryption@0.5.7
...
- Updated dependencies
  ...
  - @maroonedsoftware/encryption@0.5.6
...
- c8b0db5: Verify the JWT `aud` (audience) claim on incoming tokens.
...
- Updated dependencies [dfe5304]
  - @maroonedsoftware/encryption@0.5.5
  - @maroonedsoftware/logger@1.1.3
  - @maroonedsoftware/policies@0.5.3
  - @maroonedsoftware/cache@0.3.0
...
- Updated dependencies [b00d9b4]
  ...
  - @maroonedsoftware/cache@0.2.2
  - @maroonedsoftware/encryption@0.5.4
...
- Updated dependencies [b759188]
  - @maroonedsoftware/cache@0.2.1
  - @maroonedsoftware/encryption@0.5.3
...
- Updated dependencies [5aa38f5]
  - @maroonedsoftware/cache@0.2.0
...
- Updated dependencies [3422e87]
  - @maroonedsoftware/encryption@0.5.2
...
- 700284a: Extract `getRedirectHtml` from `EmailFactorService` into a new standalone `HtmlRedirectProvider`. Use it anywhere you want to land the browser somewhere after a non-idempotent side effect without exposing the destination to a crawler — the email magic link flow, an OIDC `completeAuthorization` handler, etc.
...
- cc1164e: Add `OidcProviderConfig.allowInsecureIssuer` to permit `http://` issuer URLs against a local IdP (docker-compose Keycloak, etc.) by wiring `openid-client`'s `allowInsecureRequests` execute hook into discovery. A warning is logged whenever the flag is enabled — including on `https://` issuers where it has no effect — so it doesn't get left on by accident. Do not enable in production.
...
- Updated dependencies [108c1d4]
  - @maroonedsoftware/encryption@0.5.0
  - @maroonedsoftware/errors@1.7.0
  - @maroonedsoftware/cache@0.1.5
...
- 8232ee3: Add session-level refresh tokens with rotation + theft detection, a session
...
- 54dbb7c: Add optional `label` to `MfaEligibleFactor`
...
- f8a0156: Add MFA orchestration on top of the per-factor services.
...
- 47c201a: Replace `Pbkdf2PasswordHashProvider` with `Argon2idPasswordHashProvider` as the bundled `PasswordHashProvider` implementation.
