...
| `zodErrorDetails`       | function | `(error: ZodError) => Record<string, string \| string[]>`                                              | The field map `parseAndValidate` puts on the thrown error, for callers that ran the schema themselves and need an identical body (a synchronous validator, for instance). |
