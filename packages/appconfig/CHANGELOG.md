...
- 7587006: Widen optional SDK peer ranges to allow the new majors now tested against: ioredis `^5.10.1 || ^6.0.0` (cache), @google-cloud/storage `^7.14.0 || ^8.0.0` (storage), and @google-cloud/secret-manager `^6.1.1 || ^7.0.0` (appconfig).
...
- 3ce5586: Add `AppConfigSourceEnv`, which reads the process environment as a configuration layer.
...
- 7501d83: Widen a literal `defaultValue` to its base primitive in `AppConfig.get` for loosely-typed configs.
...
- 6568f36: Remove the coercing `AppConfig` accessors: `getString`, `getNumber`, `getBoolean`, and `getObject`.
