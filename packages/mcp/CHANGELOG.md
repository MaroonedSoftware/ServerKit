...
- 88a601e: A failed tool call can be explained to the model instead of failing the protocol.
...
- 179f5bd: MCP handlers can reach the request's scoped DI container.
...
- 87ec6c3: Add `McpAuthenticationHandler`, so the MCP shared bearer token resolves an `AuthenticationSession`
