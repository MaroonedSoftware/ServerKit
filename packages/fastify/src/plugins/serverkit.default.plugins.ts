 ...
 *
 * The stack is: {@link errorPlugin} → {@link serverKitContextPlugin} → (optional
 * {@link rateLimiterPlugin}) → {@link corsPlugin} → {@link authenticationPlugin}.
 * The rate limiter is inserted only when a {@link RateLimiter} is registered in the container,
 * so apps that never bind one skip it automatically. Plugins load in registration order and
...
export const serverKitDefaultPlugins = (container: Container, options?: ServerKitDefaultPluginsOptions): ServerKitPlugin[] => {
  const plugins: ServerKitPlugin[] = [errorPlugin(container), serverKitContextPlugin(container)];
