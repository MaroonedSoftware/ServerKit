...
- 6152d6d: Add `JobWorkerPolicy` and `PermanentJobError`, and settle each job in a batch on its own.
...
- 528f24e: `PgBossJobRunner` now runs each job execution in its own scoped container instead of resolving jobs from the root container.
...
- 9df92a6: Add `KyselyTransactionConnectionProvider` to `@maroonedsoftware/jobbroker/pgboss`.
