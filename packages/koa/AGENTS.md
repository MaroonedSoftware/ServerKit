...
| Export                              | Kind                       | Shape                                                                                          | Notes                                                                                                                                                                                                                                                                                                     |
| ----------------------------------- | -------------------------- | ---------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `errorMiddleware`                   | function                   | `() => ServerKitMiddleware`                                                                    | **Must be first.** Maps errors to responses; emits `error`/`warn` on the app with `ctx`. The builder logs each with `method`, `path` (never the query string), `status`, `requestId`, and `correlationId`; a 4xx `HttpError` logs at `warn`, not `error`.                                                 |
...
| `ServerKitServerBuilder`    | class     | `setup(config, logger, modules, parserMappings?)`, `setupMiddleware(fn?)`, `setupRoutes(routers)`, `start(port, options?)`, `whenReady()`, `get lifecycleSignal` | Extends `ServerKitServerBuilderBase` from `servercore`. Sets Luxon's default zone to UTC on construction. `start` resolves after the `start` hooks. |
