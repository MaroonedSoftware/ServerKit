...
/**
 * A Server-Sent Events stream over a Koa response.
 *
 * SSE is a streaming (non-JSON) body, so the stream takes over the raw socket
 * (`ctx.respond = false`) and writes frames directly rather than handing Koa a stream body:
 * Koa pipes stream bodies with `stream.pipeline`, which logs `ERR_STREAM_PREMATURE_CLOSE` on
 * every client disconnect. Owning the socket lets us swallow that expected teardown.
 *
 * Transport only — it knows nothing about what is being streamed. See
 * `@maroonedsoftware/koa/serverfeed` for the realtime-bus feed built on top of it.
 */
...
/**
 * The subset of the ServerKit/Koa context an SSE stream touches. Kept as a small structural
 * type so handlers are unit-testable with a fake ctx (a fake `res` sink) without standing up
 * the whole server.
 */
...
/**
 * Open an SSE stream on a Koa context: set the streaming headers, take over the raw socket,
