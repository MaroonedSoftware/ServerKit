...
| ------------------------------------ | --------------- | ----------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ServerkitError`                     | class           | `new ServerkitError(message: string, options?: { cause?: unknown })`                                        | Extends `Error`. Fields: `details?`, `cause?`, `internalDetails?`. Restores the prototype via `new.target`. `options.cause` is re-assigned after `super` — see the gotcha.              |
...
| `HttpError`                          | class           | `new HttpError(statusCode: HttpStatusCodes, message?: HttpStatusMessage<…>, options?: { cause?: unknown })` | Extends `ServerkitError`. Adds `statusCode` (readonly) and `headers?`. Defaults the message from the status. Inherits the `new.target` prototype restore — do not pin it in a subclass. |
...
| `httpError`                          | factory         | `(statusCode, message?, options?) => HttpError`                                                             | Preferred over `new HttpError(...)`; infers the message type from the status. Forwards `options.cause`.                                                                                 |
...
| `HttpStatusMap`                      | constant        | `Record<HttpStatusCodes, string>` — status code to default message                                          | The runtime table behind `HttpStatusCodes`; `code in HttpStatusMap` narrows a `number` to one `httpError` accepts.                                                                      |
...
- **Never call `Object.setPrototypeOf(this, MyClass.prototype)` in a subclass constructor.**
  `ServerkitError` already restores the prototype from `new.target`, which is the _actual_
  constructor being invoked. Pinning it to a fixed class afterwards overwrites that and breaks
  `instanceof` for anything that subclasses yours — `HttpError` did exactly this, which is why its
  own subclasses had to repeat the workaround. Inherit and do nothing.
- **`cause` is re-assigned after `super()`, deliberately.** The declared `cause` field compiles to a
  `defineProperty` under `useDefineForClassFields`, which runs after the super call and would
  otherwise overwrite the native `Error.cause` with `undefined`. Do not "simplify" this to
  `declare cause?: Error`: the plain assignment is what keeps `cause` an own **enumerable**
  property, matching `withCause`, whereas the native one `super` installs is non-enumerable and
  disappears from anything that spreads or enumerates the error.
