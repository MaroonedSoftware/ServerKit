...
describe('explain — operators', () => {
  ...
  it('records direct tuples examined, including the matched one', async () => {
    const repo = new InMemoryTupleRepository([parseTuple('doc:d1.viewer@user:carol'), parseTuple('doc:d1.viewer@user:alice')]);
  ...
  it('traces a tupleToUserset path through a parent folder', async () => {
    const repo = new InMemoryTupleRepository([parseTuple('doc:d1.parent@folder:eng'), parseTuple('folder:eng.viewer@user:bob')]);
  ...
  it('exclusion denies when subtract is allowed', async () => {
    const repo = new InMemoryTupleRepository([parseTuple('doc:d1.viewer@user:alice'), parseTuple('doc:d1.banned@user:alice')]);
  ...
  it('follows userset subjects into a child trace', async () => {
    const repo = new InMemoryTupleRepository([parseTuple('doc:d1.editor@folder:eng.viewer'), parseTuple('folder:eng.viewer@user:alice')]);
