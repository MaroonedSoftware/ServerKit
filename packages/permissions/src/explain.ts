...
export type CheckTrace =
  DirectTrace | ComputedTrace | TupleToUsersetTrace | UnionTrace | IntersectionTrace | ExclusionTrace | CycleTrace | MaxDepthTrace | CachedTrace;
...
const traceCheck = async (ctx: ExplainCtx, object: ObjectRef, relOrPerm: string, subject: SubjectRef, depth: number): Promise<CheckTrace> => {
...
const traceDirect = async (ctx: ExplainCtx, object: ObjectRef, relation: string, subject: SubjectRef, depth: number): Promise<DirectTrace> => {
  ...
  for (const t of stored) {
    if (subject.kind === 'concrete' && t.subject.kind === 'concrete' && t.subject.namespace === subject.namespace && t.subject.id === subject.id) {
      matched = t;
      allowed = true;
      break;
    }
