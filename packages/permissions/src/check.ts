...
const evaluate = async (
  ...
  switch (expr.kind) {
    ...
    case 'exclusion':
      return (
        (await evaluate(ctx, object, relation, expr.base, subject, depth)) && !(await evaluate(ctx, object, relation, expr.subtract, subject, depth))
      );
  }
};

const hasDirectTuple = async (ctx: CheckCtx, object: ObjectRef, relation: string, subject: SubjectRef, depth: number): Promise<boolean> => {
