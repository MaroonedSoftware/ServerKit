...
- **Throw synchronously if `MultipartBody.parse()` is called more than once.** The request body can only be consumed once; the second call used to hang. It now throws a `ServerkitError` immediately. `BusboyWrapper.parse()` carries the same single-shot guard.
